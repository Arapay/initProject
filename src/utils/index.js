...
/**
 * eCharts 自定义提示框 数据统一保留小数
 * @param params
 * @param config
 * @returns String
 */
export function chartsFormatter(params, config = {}) {
  const defaultConfig = {
    axisValue: { visible: true },
    marker: { visible: true },
    seriesName: { visible: true },
    value: { visible: true, fixed: { visible: true, num: 2 }}
  }
  Object.assign(defaultConfig, config)

  const title = defaultConfig.axisValue.visible ? `${params[0].axisValue}</br>` : ''

  let data = ''
  params.forEach((item) => {
    const marker = defaultConfig.marker.visible ? item.marker : ''
    const seriesName = defaultConfig.seriesName.visible ? item.seriesName : ''
    const value = defaultConfig.value.visible
      ? defaultConfig.value.fixed.visible
        ? item.value.toFixed(defaultConfig.value.fixed.num)
        : item.value
      : ''
    data += `${marker} ${seriesName} ${value}</br>`
  })
  return title + data
}
...
/**
 * eCharts 颜色统一配置
 * @param params 默认返回数组长度为4的颜色数组
 * @returns Array
 */

export function setColor(params) {
  params = params || 'default'
  const colorJson = {
    default: ['#d06d34', '#1a8699', '#1d8955', '#aea023'],
    two: ['#E1CD25', '#0FCFFE']
  }
  return colorJson[params]
}
